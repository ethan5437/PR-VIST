...
def build_glove_voc(threshold, vocab, paragraph):
    data_path = '../glove/glove.42B.300d.{}'
...
def build_vocab(text, threshold, coco):
    ...
    print(text[2])
    dialog = json.load(open(text[2]))
    new_dialog = []
    for story in dialog:
        for image in story:
            new_dialog.append(image)
    dialog=new_dialog
    print(len(dialog))
    for i, dia in enumerate(dialog):
        if i % 5000 == 0 :
...
def main(args):
    ...
    vocab = build_vocab(text=[args.caption_path, args.caption_path_1, args.caption_path_2],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--caption_path_2', type=str,
                        default="../data/Visual-Genome/visual_genome_paragraph_train.json", help='path for train annotation file')
