...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.args = args
        if args.ethan_position or args.abs_position:
            self.pos_embedding = nn.Embedding(30, args.word_embedding.shape[1])
        if args.recurrent_UHop:
            self.sen_pos_embedding = nn.Embedding(30, args.word_embedding.shape[1])

    def reduce_prev(self, rela_previous, word_previous, question_out, rela_previous_sen=None, word_previous_sen=None):    
#         if args.recurrent_UHop and rela_previous_sen!=None and word_previous_sen!=None:
#             atten_previous = self.abwim_atten(question_out, word_previous, rela_previous, word_previous_sen, rela_previous_sen)
#         else:
    ...
    def abwim_atten(self, question_out, word_relation, rela_relation, word_relation_sen=None, rela_relation_sen=None, dump_atten=False):
...
#         if args.recurrent_UHop and word_relation_sen!=None and rela_relation_sen!=None:
#             rela_relation_sen = torch.transpose(rela_relation_sen, 0, 1)
#             word_relation_sen = torch.transpose(word_relation_sen, 0, 1)
#             rela_relation_sen = self.sen_pos_embedding(rela_relation_sen)
#             word_relation_sen = self.sen_pos_embedding(word_relation_sen)
            
#             rela_relation = rela_relation + rela_relation_sen
#             word_relation = word_relation + word_relation_sen
    ...
    def forward(self, *inputs):
        ...
        if self.args.dataset == 'vist':
            ques_position = inputs[5]
#             if self.args.recurrent_UHop:
#                 word_previous_sen = inputs[6]
#                 rela_previous_sen = inputs[7]
#         word_previous = word_previous[-10:]
#         rela_previous = rela_previous[-10:]
...
#         if self.dataset == 'vist':
        if self.args.recurrent_UHop:
            ques_position = torch.transpose(ques_position, 0, 1)
            ques_position = self.sen_pos_embedding(ques_position)
            question = question + ques_position
            
        elif self.args.ethan_position or self.args.abs_position:
            ques_position = torch.transpose(ques_position, 0, 1)
            ques_position = self.pos_embedding(ques_position)
            question = question + ques_position
...
#         if self.args.recurrent_UHop:
#             for word_prev, rela_prev, word_prev_sen, rela_prev_sen in zip(word_previous, rela_previous, word_previous_sen, rela_previous_sen):
#                 if rela_prev.shape[0]>0:
#                     question_out = self.reduce_prev(rela_prev, word_prev, question_out, rela_prev_sen, word_prev_sen)
#         else:
